...
class ChessModel(nn.Module):
    def __init__(self):
        ...
        self.conv4 =  nn.Conv2d(n_conv_layers, n_conv_layers, (3,3), 1) # 6*6 -> 4*4 + res3
        self.bn4 = nn.BatchNorm2d(n_conv_layers)
        self.dp3 = nn.Dropout2d(p=0.1)
        self.pad4 = torch.nn.ZeroPad2d(1) # 4*4 -> 6*6

        self.conv5 =  nn.Conv2d(n_conv_layers, n_conv_layers, (3,3), 1) # 6*6 -> 4*4 + res3
        self.bn5 = nn.BatchNorm2d(n_conv_layers)
        self.dp4 = nn.Dropout2d(p=0.1)

        self.deconv0 = nn.ConvTranspose2d(n_conv_layers, n_conv_layers, (3,3), 1) # 4*4 -> 6*6
        self.bn6 = nn.BatchNorm2d(n_conv_layers)
    ...
    def forward(self, x):
        ...
        out = self.conv4(out+res1+res3)
        out = self.bn4(out).relu()
        out = self.dp3(out)
        out = self.pad4(out)

        out = self.conv5(out+res2+res4)
        out = self.bn5(out).relu()
        out = self.dp4(out)

        out = self.deconv0(out)
        out = self.bn6(out).relu()
